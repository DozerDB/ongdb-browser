...
const slides = [
  ...
  <Slide>
    ...
    <div className='col-sm-9'>
      <h4>Load records</h4>
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/products.csv" AS row
CREATE (n:Product)
SET n = row,
...
n.reorderLevel = toInteger(row.reorderLevel), n.discontinued = (row.discontinued <> "0")`}</pre>
      ...
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/categories.csv" AS row
CREATE (n:Category)
SET n = row`}</pre>
      </figure>
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/suppliers.csv" AS row
CREATE (n:Supplier)
SET n = row`}</pre>
      </figure>
...
RETURN DISTINCT s.companyName as ProduceSuppliers`}</pre>
  ...
  <Slide>
    ...
    <div className='col-sm-9'>
      ...
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/customers.csv" AS row
CREATE (n:Customer)
SET n = row`}</pre>
      </figure>
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/orders.csv" AS row
CREATE (n:Order)
SET n = row`}</pre>
      </figure>
      <figure>
...
CREATE (c)-[:PURCHASED]->(o)`}</pre>
  ...
  <Slide>
    ...
    <div className='col-sm-9'>
      <h4>Load and index records</h4>
      <figure>
        <pre className='pre-scrollable code runnable'>{`LOAD CSV WITH HEADERS FROM "http://cdn.graphfoundation.org/data/northwind/order-details.csv" AS row
